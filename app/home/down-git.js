...
downGitModule.factory("downGitService", [
    ...
    function ($http, $q) {
        ...
        var mapFileAndDirectory = function (
            dirPaths,
            files,
            requestedPromises,
            progress
                ...
                for (var i = files.length - 1; i >= 0; i--) {
                    zip.file(
                        repoInfo.rootDirectoryName +
                            files[i].path.substring(
                                decodeURI(repoInfo.resPath).length + 1
                            ),
                        files[i].data
                    );
                }
        ...
        var getFile = function (path, url, files, requestedPromises, progress) {
            var promise = $http.get(url, { responseType: "arraybuffer" }).then(
                function (file) {
        ...
        var downloadFile = function (url, progress, toastr) {
            ...
            $http.get(url, { responseType: "arraybuffer" }).then(
                ...
                function (error) {
                    console.log(error);
                    progress.isProcessing.val = false;
                    toastr.warning("无法完成任务，服务器失效或URL错误。", {
